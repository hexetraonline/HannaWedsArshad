...
function setupDateScratchCard() {
  ...
  function paintCover() {
    ...
    const cover = context.createLinearGradient(0, 0, rect.width, 0);
    cover.addColorStop(0, "#d8a76a");
    cover.addColorStop(0.5, "#f4cf8a");
    cover.addColorStop(1, "#c5964f");
    context.globalCompositeOperation = "source-over";
    context.fillStyle = cover;
    context.fillRect(0, 0, rect.width, rect.height);

    const glow = context.createRadialGradient(rect.width * 0.48, rect.height * 0.5, 0, rect.width * 0.48, rect.height * 0.5, rect.width * 0.42);
    glow.addColorStop(0, "rgba(255, 245, 214, 0.46)");
  ...
  function revealDate() {
    ...
    countdown.classList.add("is-unlocked");
...
async function startMusic() {
  ...
  try {
    ...
    musicToggle.title = "Music could not play. Check that audio.mp3 is in this folder.";
    console.warn("Wedding music could not play:", error);
  }
}
...
weddingMusic.addEventListener("error", () => {
  ...
  musicToggle.classList.remove("is-playing");
  musicToggle.classList.add("has-error");
  musicToggle.title = "Music file was not found or could not be loaded.";
});
